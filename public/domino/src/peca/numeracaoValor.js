function adicionarNumeracaoDasPecas(props) {

    const divPeca = props.peca;
    ...
    function criarElementoPontoDeDomino(indice, lado) {
        if (temPontoDeDomino(indice, lado)) {
            ...
            const leftPosition = (indice % 3) * 33.33 + 7.3;
            const topPosition = Math.floor(indice / 3) * 33.33 + 10.33;

            pontoDeDomino.style.left = `calc(${leftPosition}%)`;
            pontoDeDomino.style.top = `calc(${topPosition}%)`;
    ...
    const pecaInferior = divPeca.querySelector(`.metade-${ladoInferior}`);
    pecaInferior.appendChild(containerInferior);

    var lado = "superior";
    const peca = divPeca.querySelector(`.metade-${lado}`);
    peca.appendChild(container);

    return divPeca;
}

export { adicionarNumeracaoDasPecas };